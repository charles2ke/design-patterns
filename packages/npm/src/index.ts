...
export function matchesQuery(pattern: Pattern, query: string): boolean {
  ...
  return [pattern.name, pattern.intent, pattern.useWhen, pattern.category].some((field) =>
...
export function filterPatterns(
  ...
  return source.filter(
    (pattern) =>
      (category === 'All' || pattern.category === category) && matchesQuery(pattern, query),
  );
}

export function countByCategory(source: Pattern[]): Record<PatternCategory, number> {
  return source.reduce(
    (acc, pattern) => {
      acc[pattern.category] += 1;
      return acc;
    },
    { Creational: 0, Structural: 0, Behavioral: 0 } as Record<PatternCategory, number>,
  );
}
